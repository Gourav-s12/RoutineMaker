...
router.route('/api/createTimeTable.json').all(function (req, res) {
  ...
  connection.beginTransaction(function (err) {
    ...
    connection.query("INSERT INTO college.subject VALUES(null,'Networking','IV',3,0,1,'..Slot Pref..','" + req.query.time_schedule_name + "');");
    connection.query("INSERT INTO college.subject VALUES(null,'Microprocessor','IV',3,0,1,'..Slot Pref..','" + req.query.time_schedule_name + "');");
    connection.query("INSERT INTO college.subject VALUES(null,'Microprocessor Lab','IV',3,1,1,'..Slot Pref..','" + req.query.time_schedule_name + "');");

    connection.query("INSERT INTO college.teacher VALUES(null,'G.D.',5,'..Time Matirx..','" + req.query.time_schedule_name + "');");
    connection.query("INSERT INTO college.teacher VALUES(null,'B.K.',5,'..Time Matirx..','" + req.query.time_schedule_name + "');");
...
router.route('/api/generateTimeTable.json').all(function (req, res) {
  ...
  connection.beginTransaction(function (err) {
    ...
    connection.query("SELECT * FROM college.subject WHERE fk_time_schedule_name ='" + req.query.time_schedule_name + "';", function (err, result, fields) {
      ...
      gen.sub = [];
      result.forEach(elem => {
        gen.sub.push(new gen.Sub(elem.sub_id, elem.sub_name, elem.fk_sem_no, Number(elem.hrs_per_week), Boolean(Number(elem.lab_or_not)), [
    ...
    connection.query("SELECT * FROM college.teacher WHERE fk_time_schedule_name ='" + req.query.time_schedule_name + "';", function (err, result, fields) {
      ...
      gen.teach = [];
      result.forEach(elem => {
        gen.teach.push(new gen.Teach(elem.tr_id, elem.tr_name, elem.max_continuois_period, [
          [true, true, true, true, true, true],
          [true, true, true, true, true, true],
          [true, true, true, true, true, true],
          [true, true, true, true, true, true],
    ...
    connection.query("SELECT * FROM college.sub_teacher WHERE fk_time_schedule_name ='" + req.query.time_schedule_name + "';", function (err, result, fields) {
      ...
      for (var i = 0; i < gen.subTeach.length; i++)
        gen.subTeach[i] = [gen.sub.map(o => o.id).indexOf(gen.subTeach[i][0]), gen.teach.map(o => o.id).indexOf(gen.subTeach[i][1])];
    });
    connection.commit(function (err) {
      if (err) {
        throw err;
      ...
      res.send({
        status: "200",
        result: gen.getResultArr()  //arr[routine no][sem no][day no][slot no]
      }); 
    });
  });
});
