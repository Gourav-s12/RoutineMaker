...
class Day {
    constructor(day_name, full) {
        ...
        if (full == T)
            ...
            this.slots = module.exports.slot_count_half;
    }
}
...
module.exports.slot_count_half = Math.floor(module.exports.slot_count/2);
...
function pre_process() {
    ...
    posi = new Array(module.exports.sem.length);   //[Sem][Day][Slot]
    for (let i = 0; i < module.exports.sem.length; i++) {
        posi[i] = new Array(module.exports.days.length);
        for (let j = 0; j < module.exports.days.length; j++) {
            posi[i][j] = new Array(module.exports.days[j].slots);
			for (let k = 0; k < module.exports.days[j].slots; k++) {
				posi[i][j][k] = [];
			}
        }
    }
    ...
    for (let s = 0; s < module.exports.sem.length; s++) {
        for (let x = 0; x < module.exports.days.length; x++) {
			for (let y = 0; y < module.exports.days[x].slots; y++) {
                for(let i = 0;i < module.exports.sub.length;i++) {
                    let posible = true;
                    if(module.exports.sub[i].slot_pref[x][y] != true) posible=false;

                    let teach_not_available = false;
                    for (let j = 0; j < subToTeacher(i).length; j++) {
                        if (module.exports.teach[subToTeacher(i)[j]].time_matirx[x][y] == false) {
...
function gen(s, x, y) {
    if(res.length >= 10000000000000000) return true;
    ...
    if (x >= module.exports.days.length) return false;
    if (y >= module.exports.days[x].slots) return false;
    if (s >= module.exports.sem.length) return false;

    var hash = lib.getHashCodeCuston(s,x,y,module.exports.sub,arr,T_reserved);
    if(map[hash] == true) return false;
    map[hash] = true;
    ...
    let q = posi[s][x][y];
    q = lib.shuffle(q);
    //Push Labs at back at first
    for (let i = 0; i < module.exports.sub.length; i++) {
        if (module.exports.sub[i].lab == T) {
            q = lib.arrRemove(q, i);
            q.push(i);
        }
    }
    ...
    if (arr[s][x][y - 1] != null && y != module.exports.slot_count_half) {
        var r = 6;
        if(arr[s][x][y - 2] != null && arr[s][x][y - 2] == arr[s][x][y - 1]) r+=4;
        if(lib.rand(1,10) > r) {
            q = lib.arrRemove(q, arr[s][x][y - 1]);
            q.unshift(arr[s][x][y - 1]);
        }
    }
    //Push labs at front if past break time and there are class available
    for (let i = 0; i < module.exports.sub.length; i++) {
        if (module.exports.sub[i].lab == T && y >= module.exports.slot_count_half && module.exports.sub[i].slots_per_week >= (module.exports.slot_count - y)) {
            q = lib.arrRemove(q, i);
            q.unshift(i);
        }
    }
    //Run wihout allocating any subject to this slot first if previous was null
    if(force_sel.times <= 0 && y != 0  && y != module.exports.slot_count_half && arr[s][x][y-1] == null) {
        arr[s][x][y] = null;
    ...
    for (const i of q) {
        ...
        if (force_sel.times > 0) {
            if (force_sel.day != x  || y == module.exports.slot_count_half) return false;
        ...
        if (force_sel.times <= 0) {
            force_sel.day = x;
            force_sel.sub_idx = i;
            force_sel.times = Math.min(module.exports.sub[i].min_slot_length - 1,module.exports.sub[i].slots_per_week);
    ...
    if (force_sel.times > 0 || (force_sel.times <= 0 && y != 0  && y != module.exports.slot_count_half && arr[s][x][y-1] == null)) return;
    arr[s][x][y] = null;
    if (y + 1 != module.exports.days[x].slots)
        gen(s, x, y + 1);
    else
...
module.exports.showRes = () => {
    ...
    for (let i = 0; i < res.length; i++) {
        console.log("Sem " + module.exports.sem[i] + ":");
        for (let j = 0; j < res[i].length; j++) {
            for (let k = 0; k < res[i][j].length; k++) {
                ...
                for (let m = 0; m < subToTeacher(res[i][j][k]).length; m++) {
                    if (m >= 1) teach_str += ",";
                    teach_str += module.exports.teach[subToTeacher(res[i][j][k])[m]].tr_name;
                }
                process.stdout.write(module.exports.sub[res[i][j][k]].sub_name + "(" + teach_str + ")  ");
            }
            console.log("");
        }
    }
    ...
    for (let i = 0; i < res.length; i++) {
        for (let j = 0; j < res[i].length; j++) {
            for (let k = 0; k < res[i][j].length; k++) {
                ...
                for (let m = 0; m < subToTeacher(res[i][j][k]).length; m++) {
                    if (m >= 1) teach_str += ",";
                    teach_str += module.exports.teach[subToTeacher(res[i][j][k])[m]].tr_name.trim();
                }
                res2[i][j][k] = module.exports.sub[res[i][j][k]].sub_name.trim() + "(" + teach_str + ")";
            }
        }
    }
    return res2;
