...
router.route('/api/updateSem.json').all(function(req,res){
  ...
  var obj8 ={};
  var sql1="DELETE FROM college.sem ;"
    ...
    if(err){
     throw err
      ...
      for(var i =0 ;i<req.query.list.length; i++){
        connection.query("INSERT INTO college.sem VALUES('"+req.query.list[i]+"','"+req.query.time_schedule_name+"');",function(err,result) )
      }
      console.log(result);
      if(result.affectedRows == 0){
        obj8.status ="401";
      ...
      obj8.status ="200";
      obj8.message = "Successfully updated!!!";
      res.send(obj8);
      }
    }
  })
