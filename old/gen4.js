                    ...
                    if(sub[arr[s][x][y]].min_slot_length > 1 && (y+1 >= arr[s][x].length || arr[s][x][y+1] != arr[s][x][y])) {
                        ...
                        for(var i = y+1-sub[arr[s][x][y]].min_slot_length;i < y;i++) {
                            if(arr[s][x][i] != arr[s][x][y]) {
                                min_length_satisfied = false;break;
                            }
                        }
...
function generate() {
    ...
    genetic.doesABeatBFunction = function(mother,father) {
        var dif = genetic.fitness(mother)-genetic.fitness(father);
        if (dif > 0 && dif < 4000) {
            return true; 
        }
    ...
    var popu = [];
    for(var i = 0;i < 200;i++)
        popu.push(genetic.seed());
    var config = {
        crossoverFunction: genetic.crossover,
        fitnessFunction: genetic.fitness,
        population: popu,
      }
