...
module.exports.generate = () => {
    ...
    while (q.length > 0 && module.exports.res.length <= 10) {
        ...
        console.log(q.length+"  -  "+module.exports.res.length);
...
function addItems(s, x, y, arr, S_remaining, T_reserved, force_sel, greed) {
    ...
    for (let i = 0; i < module.exports.sub.length; i++) {
        if (S_remaining[i] == 0) continue;
        if (module.exports.sub[i].sem != s) continue;
        if (module.exports.sub[i].slot_pref[x][y] == F) return;
        q.push({
            s: s,
            x: x,
            y: y,
...
function checkItem(item) { //Check whether the given subject in given item can be added to routune and if yes, addItems to queue
    ...
    function allocateSub() {
        ...
        for (let i = 0; i < module.exports.sub.length; i++)
            if (S_remaining2[i] > 0 && module.exports.sub[i].sem == item.s) {
                sub_left_this_sem = T;
                break;
            }
        if (sub_left_this_sem == F) {
            if (item.s >= module.exports.sem.length - 1) {
                ...
                addItems(item.s + 1, 0, 0, arr2, S_remaining2, T_reserved2, force_sel2, item.greed);
                return;
            }
        } else if (item.y + 1 < module.exports.days[item.x].slots) {
            addItems(item.s, item.x, item.y + 1, arr2, S_remaining2, T_reserved2, force_sel2, item.greed);
            return;
        } else {
            addItems(item.s, item.x + 1, 0, arr2, S_remaining2, T_reserved2, force_sel2, item.greed);
            return;
        }
    }
}
            ...
            for (let k = 0; k < module.exports.res[i][j].length; k++) {
                for (let l = 0; l < module.exports.res[i][j][k].length; l++) {
                    ...
                    for (let m = 0; m < subToTeacher(module.exports.res[i][j][k][l]).length; m++) {
                        if (m >= 1) teach_str += ",";
                        teach_str += module.exports.teach[subToTeacher(module.exports.res[i][j][k][l])[m]].tr_name.trim();
                    }
                    process.stdout.write(module.exports.sub[module.exports.res[i][j][k][l]].sub_name.trim() + "(" + teach_str + ")  ");
                }
                console.log("");
            }
        }
...
module.exports.getResultArr = () => {
    var res2 = module.exports.res; //[routine][sem][day][slot]
    for (let i = 0; i < module.exports.res.length; i++) {
        for (let j = 0; j < module.exports.res[i].length; j++) {
            for (let k = 0; k < module.exports.res[i][j].length; k++) {
                for (let l = 0; l < module.exports.res[i][j][k].length; l++) {
                    ...
                    res2[i][j][k][l] = module.exports.sub[module.exports.res[i][j][k][l]].sub_name.trim() + "(" + teach_str + ")";
                }
            }
        }
    }
