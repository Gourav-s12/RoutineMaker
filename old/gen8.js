const Genetic = require('genetic-nodejs-multithread');
...
class Schedule {
    ...
    fitness() { //We minimize this function using Genetic Algo
        ...
        for (var s = 0; s < arr.length; s++) {
            for (var x = 0; x < arr[s].length; x++) {
                for (var y = 0; y < arr[s][x].length; y++) {
                    ...
                    if (sub[arr[s][x][y]].lab && y >= slot_count_half) fit2 += 2; //Prefer Labs after break time
        ...
        if(fit < 0)
            return fit-(lib.rand(1,5) == 1 ? lib.rand(1,8)*1000 : 0);
        else
            return fit2-(lib.rand(1,5) == 1 ? lib.rand(1,8)*1 : 0);
    }
    make_arr() {
...
function generate() {
    ...
    genetic.crossover = function(mother, father) {
        ...
        for(var i = lib.rand(1,son.subAlocation.length-1);i < son.subAlocation.length;i++) {
            for(var j = 0;j < son.subAlocation[i].length;j++) {
                if(true) {
                    ...
                    //if(exchangeS != i) {
                        ...
                        // //console.log(son.subAlocation[i][j]);
                        // console.log(sonArr);
                        if(exchangeS == null || sub[exchangeS].pos.some((obj)=>{return obj.x == son.subAlocation[i][j].x && obj.y == son.subAlocation[i][j].y}) == true || lib.rand(1,2) == 1) {
                        if(exchangeS != null)  {
                    ...
                    //if(exchangeD != i) {
                        ...
                        // console.log(daughter.subAlocation[i][j]);
                        if(exchangeD == null || sub[exchangeD].pos.some((obj)=>{return obj.x == daughter.subAlocation[i][j].x && obj.y == daughter.subAlocation[i][j].y}) == true || lib.rand(1,2) == 1) {
                        if(exchangeD != null) {
        ...
        for(var i = 0;i < child.subAlocation.length;i++) {
            for(var j = 0;j < child.subAlocation[i].length;j++) {
                if(lib.rand(1,2) == 1) {
                    ...
                    var target = sub[i].pos[lib.rand(0,sub[i].pos.length-1)];
                    var exchange = arr[target.s][target.x][target.y];
                    if(exchange != null && sub[exchange].pos.some((obj)=>{return obj.x == child.subAlocation[i][j].x && obj.y == child.subAlocation[i][j].y}) == false) continue;
                    if(exchange != null) {
                    ...
                    child.subAlocation[i][j] = lib.deepCopyObject(target);
                }
            }
        }
        return child;
    ...
    genetic.evolve({"size":250,"iterations": 1000,"skip":2000,"workersCount":3, "mutation":0.4}, null);
}
