...
class Schedule {
    ...
    fitnessO() { //We minimize this function using Genetic Algo
        var fit = 0;
    ...
    fitness() { //We minimize this function using Genetic Algo
        ...
        for(var s = 0;s < arr.length;s++) {
            for(var x = 0;x < arr[s].length;x++) {
                for(var y = 0;y < arr[s][x].length;y++) {
                    if(arr[s][x][y] == null) continue;
                    if(sub[arr[s][x][y]].sem != s) throw "Wrong sem in fit function.";
                    //--Compulsory Conditions--
                    if(sub[arr[s][x][y]].slot_pref[x][y] != true) fit+=1;
...
function generate() {
    ...
    genetic.evolve({"size":250,"iterations": 500,"skip":2000,"workersCount":10, "mutation":0.8}, null);
}
